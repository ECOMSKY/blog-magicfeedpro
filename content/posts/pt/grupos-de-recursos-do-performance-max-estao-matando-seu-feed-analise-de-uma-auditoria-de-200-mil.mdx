---
slug: "grupos-de-recursos-do-performance-max-estao-matando-seu-feed-analise-de-uma-auditoria-de-200-mil"
title: "Grupos de Recursos do Performance Max Estão Matando Seu Feed: Análise de uma Auditoria de $200 mil"
description: "PMax canibaliza campanhas de Shopping promovendo SKUs de baixa margem quando seu feed carece de hierarquia product_type. Análise forense e correções que a maioria das agências ignora."
keywords:
...
3. **Uso inconsistente de `item_group_id`.** Se você vende variantes (cor, tamanho), todas as variantes devem compartilhar um `item_group_id` e ter valores `id` únicos. [Agrupamento quebrado](https://blog.magicfeedpro.com/pt/posts/variant-clustering-in-shopping-feeds-stop-cannibalizing-your-own-ads) faz o PMax tratar uma camisa azul e uma camisa vermelha como produtos não relacionados, fraturando o aprendizado.
...
Para orientações detalhadas sobre estratégia de rótulos personalizados, veja nossa análise sobre [como marcas DTC usam rótulos personalizados para controlar gastos com anúncios](https://blog.magicfeedpro.com/pt/posts/custom-label-strategy-how-3-dtc-brands-scaled-to-8-figure-roas). Esse post inclui templates de esquema de rótulos para produtos de assinatura, pacotes e matrizes de risco de margem.
...
Cada grupo de recursos tinha metas ROAS distintas (2,8×, 2,2×, 1,5×) e conjuntos criativos enfatizando diferentes proposições de valor (performance, versatilidade, preço). Isso permitiu ao PMax otimizar dentro de barreiras de margem em vez de misturar alto e baixo AOV em um único pool de aprendizado.
...
Também rastreamos desempenho não-PMax para controlar fatores externos (sazonalidade, calendários promocionais). Shopping Padrão e contribuição de social pago permaneceram estáveis ±3pp, confirmando que o aumento foi específico do PMax. O mecanismo: [melhores hierarquias `product_type`](https://blog.magicfeedpro.com/pt/posts/google-shopping-feed-optimization-2026-guide) permitiram ao PMax alocar orçamento para SKUs de maior margem, e descrições + títulos únicos melhoraram pontuações de relevância de anúncios, reduzindo CPCs em 11-17%.
